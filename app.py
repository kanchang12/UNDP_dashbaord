...
def sb_update(table: str, match: dict, data: dict) -> None:
    try:
        ...
        requests.patch(f"{SUPABASE_URL}/rest/v1/{table}", headers=_sb_headers(), params=params, json=data, timeout=10)
    except Exception as e:
...
def upload_image_to_storage(image_b64: str, report_id: str, index: int) -> Optional[str]:
    try:
        ...
        r = requests.post(
            ...
            headers={"apikey": SUPABASE_ANON_KEY, "Authorization": f"Bearer {SUPABASE_ANON_KEY}", "Content-Type": mime},
...
# ── STEP 1 — Gemini Vision reads images and fills all UNDP fields ──────────────

def step1_image_analysis(image_b64_list: List[str]) -> dict:
    default = {
        "damage_level": "minimal", "infrastructure_type": "other",
        "infrastructure_name": "", "crisis_type": "insignificant",
        "has_debris": "attention_not_required", "debris_detected": False,
        "electricity_status": "attention_not_required",
        "health_services_status": "attention_not_required",
        "pressing_needs": [], "location_description": "",
        "confidence": 0.0, "reasoning": "No images provided.",
    }
...

{
  ...
  "pressing_needs": ["pick only applicable from: food_water, cash_assistance, healthcare, shelter, livelihoods, wash, infrastructure_restoration, psychosocial_support, authority_support, other"],
  "location_description": "brief plain language description based on visible landmarks or environment",
...
"""
    ...
    for attempt in range(3):
        try:
            ...
            p = result.get("pressing_needs", [])
            result["pressing_needs"] = p if isinstance(p, list) else [p]
            return result
        except Exception as e:
            if "429" in str(e) and attempt < 2:
                logging.warning(f"429 rate limit, retrying in {attempt + 1}s")
                time.sleep(attempt + 1)
                continue
            logging.error(f"Gemini step1 failed: {e}")
            break

    return {**default, "reasoning": "AI assessment failed after retries."}
...
def step2_area_signals(lat: Optional[float], lng: Optional[float]) -> dict:
    ...
    rows = sb_select("reports", params={
        "and": (f"(lat.gte.{lat - lat_delta},lat.lte.{lat + lat_delta},"
                f"lng.gte.{lng - lng_delta},lng.lte.{lng + lng_delta},"
                f"submitted_at.gte.{two_hours_ago})"),
        "select": "lat,lng,damage_level,crisis_type", "limit": "200",
    }) or []

    nearby = [r for r in rows if r.get("lat") and r.get("lng")
    ...
    for r in nearby:
        ...
        if ct: crisis_counts[ct] = crisis_counts.get(ct, 0) + 1
    dominant = max(crisis_counts, key=lambda k: crisis_counts[k]) if crisis_counts else None

    avg_damage = "minimal" if avg_score < 1.5 else "partial" if avg_score < 2.5 else "complete"
    return {"nearby_count": len(nearby), "avg_damage": avg_damage, "dominant_crisis": dominant,
...
def step3_combined_assessment(image_analysis: dict, area_signals: dict) -> dict:
    system_prompt = (
        "You are a UNDP crisis assessment AI. Given image analysis and surrounding area signals, "
    ...
    if OPENAI_API_KEY:
        try:
            r = requests.post(OPENAI_URL,
                ...
                json={"model": "gpt-4o", "messages": [{"role": "system", "content": system_prompt},
                      {"role": "user", "content": user_content}], "max_tokens": 300, "temperature": 0.1},
                timeout=20)
            r.raise_for_status()
...
def step4_duplicate_detection(building_footprint_id: Optional[str], device_id: str, report_id: str) -> Tuple[bool, Optional[str]]:
...
def submit_report():
    ...
    area_signals   = step2_area_signals(float(lat) if lat is not None else None, float(lng) if lng is not None else None)
    ...
    record = {
        "id": report_id, "submitted_at": submitted_at,
        ...
        "pressing_needs": pressing_needs, "image_urls": image_urls,
        ...
        "area_signal_score": area_score, "is_duplicate_flagged": is_dup,
        "version_group_id": vg_id, "language_submitted": body.get("language_submitted", "en"),
        "anonymous_device_id": device_id, "user_confirmed": False,
    }
    ...
    return cors_response(json.dumps({
        "submission_id": report_id, "image_urls": image_urls,
        "is_duplicate_flagged": is_dup, "area_signals": area_signals,
        "ai_filled": image_analysis, "combined_assessment": combined,
    }), 201)
...
def confirm_report():
    ...
    allowed = ["damage_level", "infrastructure_type", "infrastructure_name", "crisis_type",
               "has_debris", "electricity_status", "health_services_status", "pressing_needs", "location_description"]
    for field in allowed:
        if field in body.get("corrections", {}):
            update_data[field] = body["corrections"][field]
...
def get_reports():
    ...
    filters = []
    if p.get("crisis_type"):     filters.append(f"crisis_type.eq.{p['crisis_type']}")
    if p.get("damage_level"):    filters.append(f"damage_level.eq.{p['damage_level']}")
    if p.get("infrastructure_type"): filters.append(f"infrastructure_type.eq.{p['infrastructure_type']}")
    if p.get("date_from"):       filters.append(f"submitted_at.gte.{p['date_from']}")
    if p.get("date_to"):         filters.append(f"submitted_at.lte.{p['date_to']}")
    params: dict = {"order": "submitted_at.desc", "limit": "2000"}
    if filters: params["and"] = f"({','.join(filters)})"
    ...
    if bbox:
        try:
            ...
            rows = [r for r in rows if r.get("lat") and r.get("lng")
                    and min_lat <= float(r["lat"]) <= max_lat and min_lng <= float(r["lng"]) <= max_lng]
...
def dashboard_stats():
    ...
    for h in range(24):
        buckets[(now - timedelta(hours=23 - h)).strftime("%H:00")] = 0
    for r in rows:
        try:
            ts = datetime.fromisoformat(r.get("submitted_at","").replace("Z","+00:00"))
    ...
    heatmap = [[float(r["lat"]), float(r["lng"]), intensity_map.get(r.get("damage_level",""), 0.5)]
               for r in rows if r.get("lat") and r.get("lng")]
    priority_rows = sorted([r for r in rows if r.get("ai_priority_score") is not None],
                           key=lambda r: float(r["ai_priority_score"]), reverse=True)[:10]
    return cors_response(json.dumps({
        "total": total, "damage_counts": damage_counts, "infra_counts": infra_counts,
        "crisis_counts": crisis_counts,
        "timeline_24h": [{"hour": k, "count": v} for k, v in buckets.items()],
        "heatmap_points": heatmap,
        "priority_queue": [{"id": r.get("id"), "lat": r.get("lat"), "lng": r.get("lng"),
                            ...
                            "submitted_at": r.get("submitted_at")} for r in priority_rows],
...
def export_data():
    ...
    if isinstance(filters_in, str): filters_in = {}
    fmt = body.get("format", "csv")
    filters = []
    if filters_in.get("crisis_type"):         filters.append(f"crisis_type.eq.{filters_in['crisis_type']}")
    if filters_in.get("damage_level"):         filters.append(f"damage_level.eq.{filters_in['damage_level']}")
    if filters_in.get("infrastructure_type"):  filters.append(f"infrastructure_type.eq.{filters_in['infrastructure_type']}")
    if filters_in.get("date_from"):            filters.append(f"submitted_at.gte.{filters_in['date_from']}")
    if filters_in.get("date_to"):              filters.append(f"submitted_at.lte.{filters_in['date_to']}")
    params: dict = {"order": "submitted_at.desc", "limit": "10000"}
    if filters: params["and"] = f"({','.join(filters)})"
    rows = sb_select("reports", params=params) or []
    if fmt == "geojson":
        return Response(json.dumps(_rows_to_geojson(rows), indent=2), status=200,
                        mimetype="application/geo+json",
                        headers={**CORS_HEADERS, "Content-Disposition": "attachment; filename=crisismap_export.geojson"})
    return _rows_to_csv_response(rows)
...
def reporter_alert():
    """
    Flutter sends photos + GPS here.
    Step 1: Save to reporter_alerts table (safety record).
    Step 2: Backend processes images through AI pipeline.
    Step 3: Save result to reports table (appears on dashboard map).
    """
    if request.method == "OPTIONS":
        return options_response()
    ...
    # STEP 1 — always save to reporter_alerts table
    sb_insert("reporter_alerts", {
        "id": str(uuid.uuid4()),
    ...
    logging.info(f"reporter_alerts: saved for {device_id}")

    # STEP 2 — process images through AI pipeline
    images_b64: List[str] = body.get("images_b64", [])
    if isinstance(images_b64, str):
        images_b64 = [images_b64]
    ...
    image_analysis = step1_image_analysis(images_b64[:4]) if images_b64 else {
        "damage_level": "minimal", "infrastructure_type": "other",
        "crisis_type": "insignificant", "has_debris": "attention_not_required",
        "electricity_status": "attention_not_required",
        "health_services_status": "attention_not_required",
        "pressing_needs": [], "location_description": "",
        "infrastructure_name": "", "confidence": 0.0,
        "reasoning": "Alert only — no images submitted.",
    }
    ...
    # STEP 3 — save to reports table so it appears on the dashboard map
    sb_insert("reports", {
        "id": report_id, "submitted_at": now_iso,
        "lat": float(lat) if lat is not None else None,
        "lng": float(lng) if lng is not None else None,
        "building_footprint_id": body.get("building_footprint_id"),
        "location_description": image_analysis.get("location_description", ""),
    ...
    logging.info(f"reports: saved {report_id} for {device_id}")

    return cors_response(json.dumps({
        "status": "received", "report_created": True,
...
def _rows_to_geojson(rows: list) -> dict:
    ...
    for r in rows:
        ...
        features.append({
            ...
            "properties": {
                "id": r.get("id"), "submitted_at": r.get("submitted_at"),
                "infrastructure_type": r.get("infrastructure_type"),
                "infrastructure_name": r.get("infrastructure_name"),
                "damage_level": r.get("damage_level"), "crisis_type": r.get("crisis_type"),
                "has_debris": r.get("has_debris"), "electricity_status": r.get("electricity_status"),
...
def _rows_to_csv_response(rows: list) -> Response:
    ...
    return Response(csv_bytes, status=200, mimetype="text/csv; charset=utf-8",
                    headers={**CORS_HEADERS, "Content-Disposition": "attachment; filename=crisismap_export.csv"})

def _count_by(rows: list, field: str) -> Dict[str, int]:
    counts: Dict[str, int] = {}
    for r in rows:
        val = r.get(field, "other") or "other"
